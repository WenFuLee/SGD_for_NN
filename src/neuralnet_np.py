...
def SCV_B6(all_pos_train_sample, all_neg_train_sample, total_train_size, num_folds, num_epochs, learning_rate, gd_tech):
    avg_accuracy_list = []
    time_list = []
    ...
    for i in range(num_epochs):
        ...
        time_list.append(sum_time_elapsed)
    ...
    return avg_accuracy_list, time_list
...
def plotPartB4(pos_train_sample, neg_train_sample, total_train_size, gd_tech):
    ...
    avg_accuracy_list1 = SCV_v2(pos_train_sample, neg_train_sample, total_train_size, 10, num_epochs, 0.01, 'SGD')
    avg_accuracy_list2 = SCV_v2(pos_train_sample, neg_train_sample, total_train_size, 10, num_epochs, 0.01, 'AdaGrad')
    avg_accuracy_list3 = SCV_v2(pos_train_sample, neg_train_sample, total_train_size, 10, num_epochs, 0.01, 'Adam')

    new_x1, new_y1 = zip(*sorted(zip(epoch_list, [i * 100 for i in avg_accuracy_list1])))
    plt.plot(new_x1, new_y1, 'ro-', label='SGD')
    global_min1 = max(min([i * 100 for i in avg_accuracy_list1]) - 1, 0)
    global_max1 = min(max([i * 100 for i in avg_accuracy_list1]) + 1, 100)

    new_x2, new_y2 = zip(*sorted(zip(epoch_list, [i * 100 for i in avg_accuracy_list2])))
    plt.plot(new_x2, new_y2, 'go-', label='AdaGrad')
    global_min2 = max(min([i * 100 for i in avg_accuracy_list2]) - 1, 0)
    global_max2 = min(max([i * 100 for i in avg_accuracy_list2]) + 1, 100)

    new_x3, new_y3 = zip(*sorted(zip(epoch_list, [i * 100 for i in avg_accuracy_list3])))
    plt.plot(new_x3, new_y3, 'bo-', label='Adam')
    global_min3 = max(min([i * 100 for i in avg_accuracy_list3]) - 1, 0)
    global_max3 = min(max([i * 100 for i in avg_accuracy_list3]) + 1, 100)

    global_min = min(min(global_min1, global_min2), global_min3)
    global_max = max(max(global_max1, global_max2), global_max3)

    plt.axis([1, epoch_list[-1], global_min, global_max])
    plt.grid(which='both')
    plt.xlabel('# of epoch')
    plt.ylabel('% of accuracy')
...
def plotPartB6(pos_train_sample, neg_train_sample, total_train_size):
    num_epochs = 130
    avg_accuracy_list1, time_list1 = SCV_B6(pos_train_sample, neg_train_sample, total_train_size, 10, num_epochs, 0.01, 'SGD')
    num_epochs = 40
    avg_accuracy_list2, time_list2 = SCV_B6(pos_train_sample, neg_train_sample, total_train_size, 10, num_epochs, 0.01, 'AdaGrad')
    num_epochs = 30
    avg_accuracy_list3, time_list3 = SCV_B6(pos_train_sample, neg_train_sample, total_train_size, 10, num_epochs, 0.01, 'Adam')

    new_x1, new_y1 = zip(*sorted(zip(time_list1, [i * 100 for i in avg_accuracy_list1])))
    plt.plot(new_x1, new_y1, 'ro-', label='SGD')
    global_min1 = max(min([i * 100 for i in avg_accuracy_list1]) - 1, 0)
    global_max1 = min(max([i * 100 for i in avg_accuracy_list1]) + 1, 100)

    new_x2, new_y2 = zip(*sorted(zip(time_list2, [i * 100 for i in avg_accuracy_list2])))
    plt.plot(new_x2, new_y2, 'go-', label='AdaGrad')
    global_min2 = max(min([i * 100 for i in avg_accuracy_list2]) - 1, 0)
    global_max2 = min(max([i * 100 for i in avg_accuracy_list2]) + 1, 100)

    new_x3, new_y3 = zip(*sorted(zip(time_list3, [i * 100 for i in avg_accuracy_list3])))
    plt.plot(new_x3, new_y3, 'bo-', label='Adam')
    global_min3 = max(min([i * 100 for i in avg_accuracy_list3]) - 1, 0)
    global_max3 = min(max([i * 100 for i in avg_accuracy_list3]) + 1, 100)
    ...
    global_min = min(min(global_min1, global_min2), global_min3)
    global_max = max(max(global_max1, global_max2), global_max3)
    time_max = min(min(time_list1[-1], time_list2[-1]), time_list3[-1])

    '''new_x = new_x1 + new_x2 + new_x3
    new_y = new_y1 + new_y2 + new_y3

    plt.xticks(new_x)
    plt.yticks(new_y)'''
    plt.axis([0, time_max, global_min, global_max])
    plt.grid(which='both')
...
def main():
    ...
    # Part B 6 - Accuracy vs Time from Part B 4
    plotPartB6(pos_train_sample, neg_train_sample, total_train_size)
